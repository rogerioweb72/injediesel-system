...
export function MarcaAccordion({ marca, readOnly = false }: Props) {
  ...
  // On mobile, always show expanded and keep brand header only as label
  ...
  return (
    ...
    >
      ...
      {/* Mobile brand label — no toggle, always visible */}
      {isMobile && (
        <div className="px-5 py-3 border-b border-white/[0.05] flex items-center justify-between">
          <h2
            className="text-sm font-black tracking-[0.08em] uppercase leading-none"
            style={{ color: '#E60000', transform: 'skewX(-6deg)', display: 'inline-block' }}
          >
            {marca.marca}
          </h2>
      ...
      {/* ── VEHICLE CARDS ── */}
      {shouldShowAsExpanded && (
        <div className={cn(isMobile ? 'px-3 pb-3 pt-2' : 'px-4 pb-4 pt-2', 'border-t border-white/[0.05]')}>
          <div className={isMobile ? 'grid grid-cols-1 gap-3' : 'grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-4'}>
            {allItems.map(item => (
              <MotorizacaoCard
                key={item.id}
